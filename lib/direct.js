...
export class DirectBackend extends EventTarget {
  ...
  async sendAndWaitFinal({ sessionKey, text, attachments = [], timeoutMs = 300000, onPartial, systemMessage, ephemeral = false }) {
    ...
    try {
      if (c.format === "anthropic") await this._callAnthropic(c, allMessages, { onDelta, signal: ac.signal, systemMessage, permanentCount: history.length });
  ...
  async _callAnthropic(c, allMessages, { onDelta, signal, systemMessage, permanentCount = 0 }) {
    ...
    // Add cache_control breakpoint on the last permanent message so Anthropic
    // caches the stable prefix (system + permanent history). Ephemeral tool context
    // after this point is the only uncached tail.
    if (permanentCount > 0 && permanentCount <= messages.length) {
      const lastPermIdx = permanentCount - 1;
      const msg = messages[lastPermIdx];
