...
export class DirectBackend extends EventTarget {
  ...
  /** No-op — kept for interface compat. Compression is unnecessary now
   *  because tool context is ephemeral (not stored in permanent history). */
  compressOldEphemerals(sessionKey) { /* no-op */ }
  ...
  async sendAndWaitFinal({ sessionKey, text, attachments = [], timeoutMs = 300000, onPartial, systemMessage, ephemeral = false }) {
    ...
    try {
      ...
      if (c.format === "anthropic") await this._callAnthropic(c, allMessages, { onDelta, signal: ac.signal, systemMessage });
      else await this._callOpenAI(c, allMessages, { onDelta, signal: ac.signal, systemMessage });

      // Store assistant reply: ephemeral if in tool loop, permanent otherwise
      if (ephemeral) {
        const ctx = this.toolContexts.get(sessionKey) || [];
        ctx.push({ role: "assistant", text: assistant, images: [] });
        this.toolContexts.set(sessionKey, ctx);
      } else {
      ...
      // Also remove from tool context if it was ephemeral
      if (ephemeral) {
        const ctx = this.toolContexts.get(sessionKey);
        if (ctx && ctx.length) ctx.pop();
      }
