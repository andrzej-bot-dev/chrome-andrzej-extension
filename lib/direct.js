...
export class DirectBackend extends EventTarget {
  ...
  /** Clear ephemeral entries from history (called at the start of each agent turn). */
  resetTurn(sessionKey) {
    const h = this.histories.get(sessionKey);
    if (!h) return;
    this.histories.set(sessionKey, h.filter((m) => !m.ephemeral));
  }

  /**
   * Compress old ephemeral entries instead of deleting them entirely.
   * Replaces large page snapshots / BROWSER_RESULTs with short summaries,
   * preserving the prefix for prompt caching while reducing token count.
   * Only compresses entries older than the most recent turn.
   */
  compressOldEphemerals(sessionKey, thresholdBytes = 40000) {
    const h = this.histories.get(sessionKey);
    if (!h) return;
    // Calculate total size
    const totalSize = h.reduce((sum, m) => sum + (m.text?.length || 0), 0);
    if (totalSize <= thresholdBytes) return; // small enough, no compression needed
    // Compress: replace old ephemeral entries with short summaries
    const compressed = [];
    for (const m of h) {
      if (m.ephemeral && m.text && m.text.length > 200) {
        // Keep first 150 chars as a summary hint
        const summary = m.text.slice(0, 150) + " […compressed…]";
        compressed.push({ ...m, text: summary });
      } else {
        compressed.push(m);
      }
    }
    this.histories.set(sessionKey, compressed);
  }

  /** Store a permanent message in history without making an LLM call. */
  storePermanent(sessionKey, role, text) {
    const h = this.histories.get(sessionKey) || [];
    h.push({ role, text, images: [], ephemeral: false });
  ...
  async sendAndWaitFinal({ sessionKey, text, attachments = [], timeoutMs = 300000, onPartial, systemMessage, ephemeral = false }) {
    ...
    history.push({ role: "user", text: text ?? "", images, ephemeral });
    ...
    try {
      if (c.format === "anthropic") await this._callAnthropic(c, history, { onDelta, signal: ac.signal, systemMessage });
      else await this._callOpenAI(c, history, { onDelta, signal: ac.signal, systemMessage });
      history.push({ role: "assistant", text: assistant, images: [], ephemeral });
      return assistant;
    } catch (e) {
      if (ac.signal.aborted) {
        // aborted by user/timeout — keep what we got
        if (assistant) history.push({ role: "assistant", text: assistant, images: [], ephemeral });
  ...
  // ---- OpenAI-compatible ----
  async _callOpenAI(c, history, { onDelta, signal, systemMessage }) {
    const messages = [];
  ...
  async _callAnthropic(c, history, { onDelta, signal, systemMessage }) {
