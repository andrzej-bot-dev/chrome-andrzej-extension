...
export class CDPController {
  ...
  async clickAtCoords(tabId, x, y, { dblclick = false } = {}) {
    ...
    // Wait for network idle after click — critical for SPAs that fetch on click
  ...
  /** Wait for network idle — no network requests for `quietMs` milliseconds. */
  async waitForNetworkIdle(tabId, quietMs = 500, timeoutMs = 5000) {
    return new Promise((resolve) => {
      ...
      const listener = (source, method) => {
        ...
        if (method.startsWith("Network.requestWillBeSent") ||
