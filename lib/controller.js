...
export class ChatController {
  ...
  constructor(deps) {
    ...
    const chat = {
      ...
      compressOldEphemerals: () => { this.d.getBackend()?.compressOldEphemerals?.(this.sessionKey); },
    ...
    this.agent = new AgentLoop({
      ...
      ui: {
        ...
        setWorking: (on, label) => this.broadcast({ t: "working", on: !!on, label: label || "" }),
  ...
  async attach(port) {
    ...
    // Determine current model label for display in chat
    let currentModel = settings.assistantName || "Andrzej";
    if (settings.backendMode === "direct") {
      const { getPreset } = await import("./providers.js");
      const p = getPreset(settings.directProvider);
      currentModel = settings.directModel || (p ? p.label : "") || "direct";
    }
    port.postMessage({
      t: "state",
      currentModel,
  ...
  addChip(label, why) {
    ...
    return {
      setResult: (ok, note) => {
        ...
        this.broadcast({ t: "chip-res", id, ok, note: note || (ok ? "ok" : "error") });
      },
    };
  }
