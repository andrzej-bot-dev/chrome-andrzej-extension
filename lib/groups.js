...
const GROUP_STATUS = {
  ...
  paused:  { title: "⏸️",  color: "grey" },   // ended incomplete (step/nudge limit) — needs "continue"
...
export class GroupManager {
   ...
   * Format: "🦞 ⏳ 3/15" — icon + status + progress, max ~20 chars.
   */
  async updateStatus(groupId, status, label = "") {
    const cfg = GROUP_STATUS[status] || GROUP_STATUS.idle;
    let title;
    if (status === "working") {
      // Extract step numbers from labels like "working… (step 3/15)" or "Worker step 3…"
      const stepMatch = label.match(/step\s*(\d+)(?:\/(\d+))?/i);
      if (stepMatch) {
        const current = stepMatch[1];
        const total = stepMatch[2] || "";
        title = `${cfg.title} ${current}${total ? "/" + total : ""}`;
      } else {
        title = `${cfg.title} …`;
      }
    } else if (status === "done") {
