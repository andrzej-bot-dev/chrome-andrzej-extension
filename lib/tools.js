...
export class BrowserTools {
  /**
   * @param {object} opts
   *  groupId            – the tab group this instance operates within
   *  getLastActiveTabId – () => the user's last active tab id in the group
   *  cdp                – optional shared CDPController (parallel workers reuse the
   *                       root controller so one `attached` Set tracks every tab)
   *  pinnedTabId        – optional: hard-pin every operation to this tab id. Used by
   *                       scoped instances so parallel workers each act on their OWN
   *                       tab without touching the group's shared target resolution.
   */
  constructor({ groupId, getLastActiveTabId, cdp = null, pinnedTabId = null }) {
    this.groupId = groupId;
    this.getLastActiveTabId = getLastActiveTabId || (() => null);
    this.cdp = cdp || new CDPController();
    this.pinnedTabId = pinnedTabId;
    this._cdpEnabled = true; // can be toggled off if user detaches debugger

    // Only the root (unpinned) instance owns the detach listener — scoped instances
    // share its CDPController, so a per-scope listener would just be a duplicate leak.
    if (!pinnedTabId) {
  ...
  /** A tab-pinned view over the same group/CDP — every op targets `tabId`. */
  scopedTo(tabId) {
    return new BrowserTools({
      groupId: this.groupId,
      getLastActiveTabId: this.getLastActiveTabId,
      cdp: this.cdp,
      pinnedTabId: tabId,
    });
  }
  ...
  async getTargetTab() {
    // Pinned instances always resolve to their own tab (parallel worker isolation).
    if (this.pinnedTabId) {
      try { return await chrome.tabs.get(this.pinnedTabId); } catch { return null; }
    }
        ...
        if (attempt < 2) {
          try {
            await chrome.scripting.executeScript({
              ...
              files: [
                "content/ocx/refs.js",
                "content/ocx/dom-utils.js",
                "content/ocx/snapshot.js",
                "content/ocx/actions.js",
                "content/content.js",
              ],
            });
  ...
  async _cdpCommand(tabId, cmd, args) {
    switch (cmd) {
      ...
      case "show_indicators":
        await this.cdp.showIndicators(tabId); return { ok: true };
      case "hide_indicators":
        await this.cdp.hideIndicators(tabId); return { ok: true };
      case "update_cursor":
        await this.cdp.updateCursor(tabId, args.x, args.y); return { ok: true };
      case "show_pill":
        await this.cdp.showPill(tabId); return { ok: true };
      case "hide_pill":
        await this.cdp.hidePill(tabId); return { ok: true };
      default:
  ...
  async navigate({ url }) {
    ...
    if (!/^https?:\/\//i.test(url) && !/^about:blank$/i.test(url)) url = "https://" + url;
    // navigate is the one op that must work FROM a restricted/blank page (you're
    // leaving it), so resolve the tab without the requireTab restriction check.
    const tab = await this.getTargetTab();
    if (!tab) throw new Error("No tabs left in the group.");

    // CDP Page.navigate does NOT activate the tab — stays in background
    if (this._cdpEnabled) {
      try {
    ...
    // Fallback: chrome.tabs.update — may activate tab in another window
    // but we have no choice without CDP
  ...
  async goBack() {
    const tab = await this.requireTab();
    // CDP: use Runtime.evaluate to call history.back() — no tab activation
    if (this._cdpEnabled) {
      try {
        await this.cdp.send(tab.id, "Runtime.evaluate", {
  ...
  async newTab({ url }) {
    // Create tab with active:false — stays in background
  ...
  /**
   * Create a background tab for a parallel worker lane and return its id.
   * Unlike newTab, this does NOT hijack `_workerTabId` (the group's single
   * target) — each lane gets an isolated tab addressed via scopedTo(tabId).
   */
  async spawnWorkerTab(url) {
    const current = await this.getTargetTab();
    const t = await chrome.tabs.create({
      url: url ? (/^https?:\/\//i.test(url) ? url : "https://" + url) : "about:blank",
      active: false,
    ...
    try { await chrome.tabs.group({ tabIds: [t.id], groupId: this.groupId }); } catch { /* group may be closing */ }
    if (url) await this.waitForTabLoad(t.id, 20000);
    return t.id;
  }

  /** Detach CDP and close a worker lane tab. Safe to call more than once. */
  async closeWorkerTab(tabId) {
    if (tabId == null) return;
    try { await this.cdp.detach(tabId); } catch { /* not attached */ }
    try { await chrome.tabs.remove(tabId); } catch { /* already gone */ }
  }

  /** Detach CDP from a worker tab but keep the tab open (for review). */
  async detachWorkerTab(tabId) {
    if (tabId == null) return;
    try { await this.cdp.detach(tabId); } catch { /* not attached */ }
  }
  ...
  static TOOL_ALIASES = {
    ...
    "read_page": "snapshot", "page_snapshot": "snapshot", "dom_snapshot": "snapshot", "get_elements": "snapshot",
    "read_text": "get_text", "extract_text": "get_text",
    ...
    "computer": "computer",  // handled specially in run()
    "navigate_to": "navigate", "goto": "navigate", "open_url": "navigate",
    "open_tab": "new_tab", "opentab": "new_tab",
    "switch_to_tab": "switch_tab", "switchtab": "switch_tab",
  };
  ...
  async run(action) {
    ...
    // Handle Claude-style `computer` tool — expand sub-actions
    if (tool === 'computer') {
      const sub = action.action; // click, type, press, scroll, screenshot, etc.
      if (sub === 'click' || sub === 'double_click') {
        tool = 'click'; args.dblclick = sub === 'double_click';
        // coordinate clicks need a ref or coordinates
        if (action.ref) args.ref = action.ref;
      } else if (sub === 'type') {
        tool = 'fill'; args.value = action.text || action.value || '';
      } else if (sub === 'press' || sub === 'key') {
        tool = 'press'; args.key = action.key || action.text || '';
      } else if (sub === 'scroll') {
        tool = 'scroll';
        if (action.direction === 'up') args.dy = -600;
        else if (action.direction === 'down') args.dy = 600;
      } else if (sub === 'screenshot') {
        tool = 'screenshot';
      } else if (sub === 'hover' || sub === 'mouse_move') {
        tool = 'hover';
      } else {
        return { ok: false, error: `Unknown computer sub-action: ${sub}` };
      }
    }
...
export function describeAction(a) {
  ...
  switch (canon) {
    ...
    case "snapshot":
    case "read_page": return `👀 Reading page layout`;
    ...
    case "spawn_workers":
    case "parallel": {
      const n = (a.subtasks || a.tasks || a.workers || a.items || []).length;
      return `🧵 Fan out ${n || "?"} parallel task${n === 1 ? "" : "s"}`;
    }
