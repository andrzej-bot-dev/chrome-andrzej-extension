...
export class BrowserTools {
  ...
  /** The tab we operate on. Priority:
   *  1. _workerTabId (set by newTab — our internal working tab)
   *  2. lastActiveTabId (set by switchTab)
   *  3. The active tab in this group (if user is browsing it)
   *  4. Most recently created tab in the group
   *  This ensures the agent keeps working on its tab even if the user switches tabs.
   */
  async getTargetTab() {
    const tabs = await chrome.tabs.query({ groupId: this.groupId });
    if (!tabs.length) return null;
    // 1. Worker tab (background tab created by newTab)
    if (this._workerTabId) {
      const wt = tabs.find(t => t.id === this._workerTabId);
      if (wt) return wt;
    }
    // 2. Last active tab (set by switchTab)
    const last = tabs.find(t => t.id === this.getLastActiveTabId());
    if (last) return last;
    // 3. Active tab in this group (user is viewing it)
    const [focused] = await chrome.tabs.query({ active: true, lastFocusedWindow: true });
    if (focused && focused.groupId === this.groupId) return focused;
    // 4. Most recently created tab
  ...
  async newTab({ url }) {
    ...
    // Remember the currently active tab so we can restore focus to it
    ...
    // On macOS, chrome.tabs.create can still focus the window even with active:false.
    // Explicitly restore focus to the previously active tab.
    if (previouslyActive && previouslyActive.id !== t.id) {
      try { await chrome.tabs.update(previouslyActive.id, { active: true }); } catch { /* tab closed */ }
    }
    // Track as internal "working tab" — don't touch the user's view
  ...
  async run(action) {
    ...
    try {
      // Aliases — LLMs sometimes invent tool names
