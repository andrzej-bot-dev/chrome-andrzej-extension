...
const READ_ONLY_TOOLS = new Set([
  ...
  "tab_info", "screenshot", "highlight", "scroll"
]);
...
- {"tool":"tab_info"} — current tab URL/title.
...
export class AgentLoop {
  ...
  async run(userText, { includePage = false, includeShot = false } = {}) {
    ...
    try {
      ...
      if (!this.preambleSentFor.has(sessionKey)) {
        body += buildPreamble(settings.assistantName) + "\n\n";
        this.preambleSentFor.add(sessionKey);
      }
      body += await this.buildContextHeader({ includePage, includeShot });
      body += `\n\n[USER MESSAGE]\n${userText}`;
      ...
      this.setGlow(true);
      await this.turnLoop(body, attachments, settings);
    } finally {
      this.running = false;
  ...
  async turnLoop(firstBody, firstAttachments, settings) {
    let body = firstBody;
    ...
    while (true) {
      ...
      const reply = await this.chat.send({ text: body, attachments });
      if (this.stopped) return;
      ...
      if (!action) return; // end of loop — plain response
      ...
      if (["click", "navigate", "back", "new_tab", "press", "fill"].includes(action.tool)) {
        await new Promise(r => setTimeout(r, 500));
        const info = await this.tools.tabInfo();
        if (info.ok) body += `\n[BROWSER_CONTEXT] Tab now: ${info.url} — "${info.title}"`;
  ...
  async approvalGate(action, settings) {
    ...
    const autopilot = fresh.actionMode === "auto" && origin && fresh.allowedSites.includes(origin);
