...
- NEVER stop the action loop until ALL items are done (or explicitly impossible).
    ...
    while (true) {
      ...
      if (decision === "deny") {
        ...
        if (result.ok === false && ["click", "fill"].includes(action.tool)) {
          ...
          if (isStaleRef && !this._justRetried) {
            this._justRetried = true;
            if (settings.debug) this.ui.systemNote(`🔄 Auto-retry: ref stale, fetching fresh snapshot…`);
            try {
