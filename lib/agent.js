...
export class AgentLoop {
  ...
  async run(userText, { includePage = false, includeShot = false } = {}) {
    ...
    try {
      ...
      // Don't hard-reset ephemerals between turns — that kills prompt cache.
      // Instead, compress old ephemeral entries (large DOM snapshots → short summaries)
      // to keep total history bounded while preserving the prefix for cache hits.
      // Full reset only happens inside compressOldEphemerals when threshold is exceeded.
      this.chat.compressOldEphemerals?.();
